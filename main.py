...
def a_star(board):
    ...
    # start_memory = sys.getsizeof(board)  # Record initial memory usage
    ...
    while priority_queue:
        ...
        if h(current_board) == 0: 
            ...
            # end_memory = sys.getsizeof(board)  # Record end memory usage
...
#         # Check block constraints (assuming 3x3 blocks)
#         start_row, start_col = 3 * (row // 3), 3 * (col // 3)
#         for i in range(3):
#           for j in range(3):
#             possible_values.discard(board[i + start_row][j + start_col])
#         constraint_count += len(possible_values)  # Higher for fewer options
